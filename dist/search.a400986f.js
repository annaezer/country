...
async function fetchCountryNameData(e) {
    ...
    try {
        const results = await _axiosDefault.default.get(URI + ENDPOINT + searchField.value);
        const oneCountry = results.data[0];
        function getCurrencies() {
            if (oneCountry.currencies[1] !== undefined) return `${oneCountry.currencies[0].name}s and ${oneCountry.currencies[1].name}s`;
            else return `${oneCountry.currencies[0].name}s`;
        }
        ...
        if (error.result.status === 404) errorMessage.textContent = "Page not found";
...
},{"./../utils.js":"60BxC","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["1fSmN","hebny"], "hebny", "parcelRequirecb08")
